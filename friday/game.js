...
function preload () {
    ...
    this.load.image('bdiamond', 'assets/PNG/Items/item007.png');
    this.load.image('ydiamond', 'assets/PNG/Items/item008.png');
    this.load.image('gdiamond', 'assets/PNG/Items/item009.png');
    this.load.image('rdiamond', 'assets/PNG/Items/item010.png');
...
function create () {
    ...
    makeDiamond(6, 4, 'bdiamond', 100);
    makeDiamond(7, 1, 'ydiamond', 200);
    makeDiamond(10, 5, 'gdiamond', 300);
    makeDiamond(0, 3, 'rdiamond', 400);

    scoreText = this.add.text(12, 8, 'Score: 0', {fontSize: '24px', fill: '#ffffff'});
...
function update () {
    ...
    scoreText.setText('Score: ' + tm.getData('score'));
}
...
function makeDiamond (x, y, key, value) {
    let diamond = collectibles.create(x*tileSize + tileSize/2, y*tileSize + tileSize/2, key);
    diamond.setData('value', value);
}

function pickupDiamond (tm, diamond) {
    tm.incData('score', diamond.getData('value'));
    pickup.play();
    diamond.destroy();
}
